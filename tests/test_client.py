...
def test_successful_get(mock_settings):
    ...
    respx.get(f"{mock_settings.base_url}/companies/1/jobs/").mock(
        return_value=Response(200, json=[{"ID": 1, "Name": "Test Job"}])
    )
...
def test_401_triggers_refresh_and_retry(mock_settings):
    ...
    token_route = respx.post(mock_settings.token_url).mock(
        return_value=Response(200, json={"access_token": "new-tok", "expires_in": 3600})
    )
    respx.get(f"{mock_settings.base_url}/companies/1/jobs/").mock(
        side_effect=[
...
def test_404_raises_not_found(mock_settings):
    ...
    respx.get(f"{mock_settings.base_url}/companies/1/jobs/999").mock(
...
def test_client_context_manager(mock_settings):
    ...
    respx.get(f"{mock_settings.base_url}/companies/1/").mock(
        return_value=Response(200, json={"ID": 1, "Name": "CVC Service"})
    )
    with SimproClient(settings=mock_settings) as client:
